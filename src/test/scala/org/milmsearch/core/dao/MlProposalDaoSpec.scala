...
class MlProposalDaoSpec extends FunSpec with ShouldMatchers
  ...
  after {
    DB.runUpdate("DROP TABLE ml_proposal", Nil)
  }
  ...
  describe("create(request) メソッドは") {
    it("作成要求に全ての値が存在し、かつ正常値の場合は、DBにそのレコードを作成する") {
      ...
      r("id")            should equal (1)
      r("proposername")  should equal ("テスト 太郎")
      r("proposeremail") should equal ("proposer@example.com")
      r("mltitle")       should equal ("MLタイトル")
      r("status")        should equal ("new")
      r("archivetype")   should equal ("mailman")
      r("archiveurl")    should equal ("http://localhost/path/to/archive/")
      r("message")       should equal ("コメント(MLの説明など)\nほげほげ)")
      r("createdat")     should equal (new Timestamp(now.getMillis))
      r("updatedat")     should equal (new Timestamp(now.getMillis))
    }
    ...
    it("""作成要求に必要最低限の値が存在した場合は、任意項目は null の状態で
      ...
      r("mltitle")       should equal ("MLタイトル")
      r("status")        should equal ("new")
      r("archivetype")   should equal (null)
      r("archiveurl")    should equal (null)
